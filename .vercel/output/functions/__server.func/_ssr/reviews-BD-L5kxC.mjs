import { i as __toESM } from "../_runtime.mjs";
import { d as require_react } from "../_libs/@radix-ui/react-alert-dialog+[...].mjs";
import { M as useContentStore } from "./router-ZY7VAfYm.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/reviews-BD-L5kxC.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var REVIEW_SERVICES = [
	"Web Development",
	"Software Development",
	"AI Solutions",
	"Automation",
	"Other"
];
/**
* Review repository.
* Public submissions enter as pending. Only approved + published appear publicly.
* Later: GET/POST/PUT/DELETE /api/reviews.
*/
var reviewsService = {
	list() {
		return useContentStore.getState().reviews;
	},
	published() {
		return useContentStore.getState().reviews.filter((r) => r.status === "approved" && r.published);
	},
	pending() {
		return useContentStore.getState().reviews.filter((r) => r.status === "pending");
	},
	getById(id) {
		return useContentStore.getState().reviews.find((r) => r.id === id) ?? null;
	},
	submit(input) {
		if (!input.clientName.trim()) throw new Error("Name is required.");
		if (!input.content.trim()) throw new Error("A review needs some words.");
		if (input.content.trim().length < 20) throw new Error("Please write a little more — at least a couple of sentences.");
		if (!Number.isFinite(input.rating) || input.rating < 1 || input.rating > 5) throw new Error("Choose a rating from 1 to 5.");
		return useContentStore.getState().createReview(input);
	},
	approve(id) {
		return useContentStore.getState().updateReview(id, {
			status: "approved",
			published: true
		});
	},
	reject(id) {
		return useContentStore.getState().updateReview(id, {
			status: "rejected",
			published: false
		});
	},
	setPublished(id, published) {
		const current = reviewsService.getById(id);
		if (!current) return null;
		if (published && current.status !== "approved") return useContentStore.getState().updateReview(id, {
			status: "approved",
			published: true
		});
		return useContentStore.getState().updateReview(id, { published });
	},
	update(id, patch) {
		return useContentStore.getState().updateReview(id, patch);
	},
	delete(id) {
		useContentStore.getState().deleteReview(id);
	}
};
function useReviews() {
	return useContentStore((s) => s.reviews);
}
function usePublishedReviews() {
	const reviews = useContentStore((s) => s.reviews);
	return (0, import_react.useMemo)(() => reviews.filter((r) => r.status === "approved" && r.published), [reviews]);
}
//#endregion
export { useReviews as i, reviewsService as n, usePublishedReviews as r, REVIEW_SERVICES as t };
