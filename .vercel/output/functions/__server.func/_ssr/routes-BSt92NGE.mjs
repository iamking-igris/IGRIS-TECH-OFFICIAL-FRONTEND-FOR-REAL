...
import { A as FUTURE_PRODUCTS_NOTE, N as listPublicProjects, _ as cn, f as services, g as ButtonLink, m as Container, s as organizationJsonLd, u as websiteJsonLd, w as useFeaturedProjects } from "./router-RsAy9zmm.mjs";
import { _ as usePublishedEcosystem } from "./router-RsAy9zmm2.mjs";
import { r as usePublishedReviews } from "./reviews-DcTRiFDH.mjs";
...
import { n as Reveal, t as ProcessSection } from "./process-section-DPmsKxDg.mjs";
import { t as ProjectRow } from "./project-row-N035qr0N.mjs";
import { a as Group, c as Mesh, d as Points, f as QuadraticBezierCurve3, g as Vector3, h as SphereGeometry, i as Float32BufferAttribute, l as MeshBasicMaterial, m as ShaderMaterial, n as BufferGeometry, o as Line, p as Scene, r as Color, s as LineBasicMaterial, t as WebGLRenderer, u as PerspectiveCamera } from "../_libs/three.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/routes-BSt92NGE.js
...
function lngLatToVector3(lng, lat, radius, out) {
...
function createCurvedArc(p1, p2, radius, pointsCount = 48) {
	const distance = p1.distanceTo(p2);
	const mid = new Vector3().addVectors(p1, p2).multiplyScalar(.5);
	const midLength = mid.length();
	const elevation = radius + Math.min(distance * .22, radius * .32);
	if (midLength > .001) mid.normalize().multiplyScalar(elevation);
	else mid.set(0, elevation, 0);
...
`;
var sphericalLineFragmentShader = `
  uniform vec3 uColor;
...
`;
var sphericalPointVertexShader = `
  uniform float uSize;
  varying vec3 vNormal;
  varying vec3 vViewPosition;
...
`;
var sphericalPointFragmentShader = `
  uniform vec3 uColor;
  uniform float uOpacity;
  varying vec3 vNormal;
  varying vec3 vViewPosition;
...
`;
function GlobalNetworkGlobe({ className }) {
	const containerRef = (0, import_react.useRef)(null);
	const canvasRef = (0, import_react.useRef)(null);
	const [activeNode, setActiveNode] = (0, import_react.useState)("SYSTEM / ACTIVE");
	(0, import_react.useEffect)(() => {
		...
		const camera = new PerspectiveCamera(42, width / height, .1, 1500);
		...
		globeGroup.rotation.x = .22;
		globeGroup.rotation.y = .38;
		scene.add(globeGroup);
		const coreGeometry = new SphereGeometry(GLOBE_RADIUS * .996, 48, 48);
		const coreMaterial = new ShaderMaterial({
			vertexShader: sphereCoreVertexShader,
			fragmentShader: sphereCoreFragmentShader,
			uniforms: {
				uBaseColor: { value: new Color(460553) },
				uRimColor: { value: new Color(15987696) },
				uRimStrength: { value: .16 }
			},
			transparent: true,
			depthWrite: true
		});
		...
		const gridMaterial = new ShaderMaterial({
			...
			uniforms: {
				uColor: { value: new Color(6052952) },
				uOpacity: { value: .2 }
			},
			transparent: true,
			depthTest: true
		});
		const equatorMaterial = new ShaderMaterial({
			vertexShader: sphericalLineVertexShader,
			fragmentShader: sphericalLineFragmentShader,
			uniforms: {
				uColor: { value: new Color(9276808) },
				uOpacity: { value: .3 }
			},
			transparent: true,
			depthTest: true
		...
		[
			...
			for (let i = 0; i <= segments; i++) {
				...
				ringPts.push(new Vector3(Math.cos(theta) * r, y, Math.sin(theta) * r));
			}
			const geom = new BufferGeometry().setFromPoints(ringPts);
			const isEquator = lat === 0;
			gridGroup.add(new Line(geom, isEquator ? equatorMaterial : gridMaterial));
		});
		[
			0,
			30,
			...
			const geom = new BufferGeometry().setFromPoints(meridianPts);
			const isPrime = lon === 0;
			gridGroup.add(new Line(geom, isPrime ? equatorMaterial : gridMaterial));
		});
		const coastGroup = new Group();
		globeGroup.add(coastGroup);
		const coastMaterial = new ShaderMaterial({
			vertexShader: sphericalLineVertexShader,
			fragmentShader: sphericalLineFragmentShader,
			uniforms: {
				uColor: { value: new Color(9276808) },
		...
		WORLD_COASTLINES.forEach((ring) => {
			...
			coastGroup.add(new Line(geom, coastMaterial));
		});
		const landPositions = [];
		...
		const landGeom = new BufferGeometry();
		landGeom.setAttribute("position", new Float32BufferAttribute(landPositions, 3));
		const landPointMaterial = new ShaderMaterial({
			vertexShader: sphericalPointVertexShader,
			fragmentShader: sphericalPointFragmentShader,
			uniforms: {
				uColor: { value: new Color(12500664) },
				uOpacity: { value: .48 },
				uSize: { value: 2 }
			},
			transparent: true,
		...
		const landPoints = new Points(landGeom, landPointMaterial);
		...
		const oceanGeom = new BufferGeometry();
		oceanGeom.setAttribute("position", new Float32BufferAttribute(oceanPositions, 3));
		const oceanPointMaterial = new ShaderMaterial({
			vertexShader: sphericalPointVertexShader,
			fragmentShader: sphericalPointFragmentShader,
			uniforms: {
				uColor: { value: new Color(6052952) },
		...
		const oceanPoints = new Points(oceanGeom, oceanPointMaterial);
		...
		NETWORK_HUBS.forEach((hub, idx) => {
			...
			for (let i = 0; i <= ringSegs; i++) {
				...
				ringPts.push(new Vector3(Math.cos(th) * ringR, Math.sin(th) * ringR, 0));
			}
			const ringGeom = new BufferGeometry().setFromPoints(ringPts);
			const ringMat = new LineBasicMaterial({
				color: 15987696,
				transparent: true,
				opacity: .4
			});
			const ringMesh = new Line(ringGeom, ringMat);
		...
		const hubGeom = new BufferGeometry();
		hubGeom.setAttribute("position", new Float32BufferAttribute(hubPositions, 3));
		const hubPointMaterial = new ShaderMaterial({
			vertexShader: sphericalPointVertexShader,
			fragmentShader: sphericalPointFragmentShader,
			uniforms: {
				uColor: { value: new Color(15987696) },
				uOpacity: { value: .95 },
				uSize: { value: 3.8 }
			},
			transparent: true,
		...
		const hubPoints = new Points(hubGeom, hubPointMaterial);
		...
		CONNECTION_ROUTES.forEach(([fromId, toId]) => {
			...
			const { curve, points } = createCurvedArc(fromObj.position, toObj.position, GLOBE_RADIUS, 40);
		...
		const packetCount = 4;
		const packets = [];
		const packetGeom = new SphereGeometry(1.3, 8, 8);
		const packetMat = new MeshBasicMaterial({
			color: 15987696,
			transparent: true,
			opacity: .95
		});
		for (let i = 0; i < packetCount; i++) {
			const mesh = new Mesh(packetGeom, packetMat.clone());
			globeGroup.add(mesh);
			mesh.visible = false;
			packets.push({
		...
		let targetRotationX = .22;
		let targetRotationY = .38;
		let rotationVelocityX = 0;
		let rotationVelocityY = .001;
		let parallaxX = 0;
		let parallaxY = 0;
		...
		function animate() {
			...
			if (!reducedMotion) {
				if (!isDragging) {
					rotationVelocityY += (.001 - rotationVelocityY) * .03;
					rotationVelocityX += (0 - rotationVelocityX) * .05;
					targetRotationY += rotationVelocityY;
					targetRotationX += rotationVelocityX;
					targetRotationX = Math.max(-.5, Math.min(.5, targetRotationX));
				}
				globeGroup.rotation.y += (targetRotationY - globeGroup.rotation.y) * .08;
				globeGroup.rotation.x += (targetRotationX - globeGroup.rotation.x) * .08;
				parallaxX += (targetParallaxX - parallaxX) * .05;
				parallaxY += (targetParallaxY - parallaxY) * .05;
				camera.position.x = parallaxX * 18;
				camera.position.y = -parallaxY * 18;
				camera.lookAt(0, 0, 0);
				pulseRings.forEach((ring) => {
					const scale = 1 + (Math.sin(time * 1.5 + ring.phase) + 1) * .5 * 1.4;
					const alpha = Math.max(0, .4 * (1 - (scale - 1) / 1.4));
				...
				packets.forEach((packet) => {
					...
					if (packet.progress >= 1) {
						...
						if (Math.random() > .5) setActiveNode(`NODE / ${arc.to.id} // ${arc.to.name.toUpperCase()}`);
					} else {
						...
						packet.mesh.material.opacity = .3 + .65 * fade;
					}
				});
			}
		...
		function onPointerMove(e) {
			...
			targetParallaxX = Math.max(-1, Math.min(1, nx)) * .2;
			targetParallaxY = Math.max(-1, Math.min(1, ny)) * .2;
			if (isDragging) {
				const deltaX = e.clientX - previousPointerX;
				const deltaY = e.clientY - previousPointerY;
				rotationVelocityY = deltaX * .005;
				rotationVelocityX = deltaY * .005;
				targetRotationY += rotationVelocityY;
				targetRotationX += rotationVelocityX;
				previousPointerX = e.clientX;
				previousPointerY = e.clientY;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		ref: containerRef,
		className: cn("group relative flex h-full w-full select-none items-center justify-center overflow-hidden p-3 sm:p-4", className),
		"aria-label": "Interactive 3D IGRIS Global Network Globe",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(243,243,240,0.03)_0%,transparent_65%)]" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", {
				ref: canvasRef,
				className: "relative z-10 block h-full w-full cursor-grab touch-pan-y active:cursor-grabbing"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "pointer-events-none absolute inset-x-5 top-4 flex items-center justify-between z-20 font-mono text-[9px] uppercase tracking-[0.18em] text-faint",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
					className: "flex items-center gap-1.5",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "inline-block h-1 w-1 bg-ink/60 rounded-full animate-pulse" }), "SYSTEM / GLOBAL"]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "text-faint/70 hidden sm:inline-block",
					children: "NETWORK / ACTIVE"
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "pointer-events-none absolute inset-x-5 bottom-4 flex items-center justify-between z-20 font-mono text-[9px] uppercase tracking-[0.18em] text-faint",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "text-faint/80",
					children: activeNode
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "hidden sm:inline-block text-faint/60",
					children: "99.98% SYNC // RT"
				})]
			})
		]
	});
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "pointer-events-none absolute inset-0 hex-grid opacity-40 md:opacity-20" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				className: "relative h-[280px] sm:h-[320px] md:col-span-6 md:h-[min(68vh,620px)]",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(GlobalNetworkGlobe, { className: "h-full w-full" })
			})]
		})]
	});
}
function useRemoteFeaturedProjects() {
	const local = useFeaturedProjects();
...
function WorkSection() {
	useFeaturedProjects();
	const { items, loading, error } = useRemoteFeaturedProjects();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("section", {
		className: "border-b border-hairline",
		id: "work",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Container, {
			...
			children: [
				...
				loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					...
					children: "Loading selected work…"
