import { n as ButtonLink } from "./button-link-DuD0WsTl.js";
import { t as Container } from "./container-CGk6eJJy.js";
import { f as useRelatedProjects, l as useProjectBySlug } from "./projects-Dck77_K-.js";
import { t as getService } from "./services-CsiE3ino.js";
import { l as absoluteUrl, n as creativeWorkJsonLd, t as breadcrumbJsonLd } from "./seo-CU7LXOqa.js";
import { c as NotFoundPage, l as useContentHydrated, r as Route } from "./router-LJGBwkzE.js";
import { t as JsonLd } from "./json-ld-IR7igJk0.js";
import { t as StatusChip } from "./status-chip-DXl3zibr.js";
import { t as ProjectVisual } from "./project-visual-CawO2wU_.js";
import { Link } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
//#region src/routes/work/$slug.tsx?tsr-split=component
function Pending({ children }) {
	return /* @__PURE__ */ jsx("p", {
		className: "mt-4 leading-relaxed text-quiet",
		children: children || "This section will be published with the case study."
	});
}
function CaseStudy() {
	const { slug } = Route.useParams();
	const hydrated = useContentHydrated();
	const project = useProjectBySlug(slug);
	const related = useRelatedProjects(slug);
	if (!project) {
		if (!hydrated) return /* @__PURE__ */ jsx("main", {
			id: "main",
			children: /* @__PURE__ */ jsxs(Container, {
				className: "pb-10 pt-28 md:pt-36",
				children: [/* @__PURE__ */ jsx("p", {
					className: "label-tech mb-5",
					children: "Work"
				}), /* @__PURE__ */ jsx("h1", {
					className: "font-display text-4xl font-semibold tracking-[-0.035em] text-quiet md:text-6xl",
					children: "Loading case study"
				})]
			})
		});
		return /* @__PURE__ */ jsx(NotFoundPage, {});
	}
	const next = related[0];
	return /* @__PURE__ */ jsxs("main", {
		id: "main",
		children: [
			/* @__PURE__ */ jsx(JsonLd, { data: breadcrumbJsonLd([
				{
					name: "Home",
					path: "/"
				},
				{
					name: "Work",
					path: "/work"
				},
				{
					name: project.title,
					path: `/work/${project.slug}`
				}
			]) }),
			/* @__PURE__ */ jsx(JsonLd, { data: creativeWorkJsonLd({
				name: project.title,
				description: project.description,
				url: absoluteUrl(`/work/${project.slug}`),
				dateCreated: project.year ? String(project.year) : null
			}) }),
			/* @__PURE__ */ jsxs("header", {
				className: "border-b border-hairline",
				children: [/* @__PURE__ */ jsxs(Container, {
					className: "pb-10 pt-28 md:pt-36",
					children: [
						/* @__PURE__ */ jsxs("p", {
							className: "label-tech mb-5",
							children: ["Work / ", project.category]
						}),
						project.placeholder && /* @__PURE__ */ jsx("div", {
							className: "mb-4",
							children: /* @__PURE__ */ jsx(StatusChip, { children: "CASE STUDY IN PREPARATION" })
						}),
						/* @__PURE__ */ jsx("h1", {
							className: "font-display text-4xl font-semibold tracking-[-0.035em] md:text-6xl",
							children: project.title
						}),
						/* @__PURE__ */ jsxs("dl", {
							className: "mt-10 grid gap-6 border-t border-hairline pt-6 text-sm sm:grid-cols-2 lg:grid-cols-4",
							children: [
								/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("dt", {
									className: "font-mono text-[10px] tracking-widest text-faint",
									children: "CLIENT"
								}), /* @__PURE__ */ jsx("dd", {
									className: "mt-1 text-ink",
									children: project.client
								})] }),
								/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("dt", {
									className: "font-mono text-[10px] tracking-widest text-faint",
									children: "CATEGORY"
								}), /* @__PURE__ */ jsx("dd", {
									className: "mt-1 text-ink",
									children: project.category
								})] }),
								/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("dt", {
									className: "font-mono text-[10px] tracking-widest text-faint",
									children: "YEAR"
								}), /* @__PURE__ */ jsx("dd", {
									className: "mt-1 text-ink",
									children: project.year ?? "—"
								})] }),
								/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("dt", {
									className: "font-mono text-[10px] tracking-widest text-faint",
									children: "STATUS"
								}), /* @__PURE__ */ jsx("dd", {
									className: "mt-1 text-ink",
									children: project.placeholder ? "In preparation" : "Published"
								})] })
							]
						})
					]
				}), /* @__PURE__ */ jsx("div", {
					className: "mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12 xl:px-16",
					children: /* @__PURE__ */ jsx(ProjectVisual, {
						visual: project.visual,
						imageUrl: project.cover_image || project.featuredImage,
						className: "mb-10 aspect-[16/8] min-h-56 w-full",
						caption: project.placeholder ? "PREPARING" : project.category.toUpperCase()
					})
				})]
			}),
			/* @__PURE__ */ jsx("article", { children: /* @__PURE__ */ jsxs(Container, {
				className: "grid gap-12 py-16 md:grid-cols-12 md:py-24",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "space-y-14 md:col-span-8",
					children: [
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Overview"
						}), /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.overview
						})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "The challenge"
						}), project.challenge ? /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.challenge
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Objectives"
						}), project.objectives.length > 0 ? /* @__PURE__ */ jsx("ol", {
							className: "mt-6",
							children: project.objectives.map((item, i) => /* @__PURE__ */ jsxs("li", {
								className: "flex gap-4 border-t border-hairline py-4 text-sm last:border-b",
								children: [/* @__PURE__ */ jsx("span", {
									className: "font-mono text-xs tracking-widest text-faint",
									children: String(i + 1).padStart(2, "0")
								}), /* @__PURE__ */ jsx("span", { children: item })]
							}, item))
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "The approach"
						}), project.approach ? /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.approach
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Design"
						}), project.design ? /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.design
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Development"
						}), project.development ? /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.development
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Key decisions"
						}), project.keyDecisions.length > 0 ? /* @__PURE__ */ jsx("ul", {
							className: "mt-6 space-y-6",
							children: project.keyDecisions.map((d) => /* @__PURE__ */ jsxs("li", {
								className: "border-t border-hairline pt-5",
								children: [/* @__PURE__ */ jsx("h3", {
									className: "font-display text-lg font-semibold",
									children: d.title
								}), /* @__PURE__ */ jsx("p", {
									className: "mt-2 text-sm leading-relaxed text-quiet",
									children: d.text
								})]
							}, d.title))
						}) : /* @__PURE__ */ jsx(Pending, {})] }),
						/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Results"
						}), /* @__PURE__ */ jsx("p", {
							className: "mt-4 leading-relaxed text-quiet",
							children: project.results ?? "Results will be published when they are real. We do not invent metrics."
						})] }),
						project.gallery.length > 0 && /* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Project gallery"
						}), /* @__PURE__ */ jsx("ul", {
							className: "mt-6 grid gap-4",
							children: project.gallery.map((image) => /* @__PURE__ */ jsx("li", { children: /* @__PURE__ */ jsx("img", {
								src: image.src,
								alt: image.alt,
								className: "w-full border border-hairline"
							}) }, image.src))
						})] }),
						project.testimonial && /* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl font-semibold",
							children: "Client note"
						}), /* @__PURE__ */ jsx("blockquote", {
							className: "mt-4 text-lg leading-relaxed text-ink",
							children: project.testimonial
						})] })
					]
				}), /* @__PURE__ */ jsx("aside", {
					className: "md:col-span-4",
					children: /* @__PURE__ */ jsxs("div", {
						className: "border border-hairline p-6 md:sticky md:top-28",
						children: [
							/* @__PURE__ */ jsx("p", {
								className: "label-tech mb-4",
								children: "Services"
							}),
							/* @__PURE__ */ jsx("ul", {
								className: "space-y-2 text-sm",
								children: project.services.length > 0 ? project.services.map((slug) => {
									const s = getService(slug);
									return s ? /* @__PURE__ */ jsx("li", { children: /* @__PURE__ */ jsx(Link, {
										to: s.href,
										className: "hover:underline",
										children: s.name
									}) }, slug) : /* @__PURE__ */ jsx("li", { children: slug }, slug);
								}) : /* @__PURE__ */ jsx("li", {
									className: "text-quiet",
									children: "To be published with the case study."
								})
							}),
							/* @__PURE__ */ jsx("p", {
								className: "label-tech mb-4 mt-8",
								children: "Technology"
							}),
							/* @__PURE__ */ jsx("p", {
								className: "text-sm text-quiet",
								children: project.technologies.length > 0 ? project.technologies.join(", ") : "To be published with the case study."
							}),
							project.url && /* @__PURE__ */ jsxs("a", {
								href: project.url,
								className: "mt-8 inline-flex items-center gap-2 text-sm",
								target: "_blank",
								rel: "noreferrer",
								children: ["Visit project", /* @__PURE__ */ jsx("span", {
									"aria-hidden": true,
									children: "→"
								})]
							})
						]
					})
				})]
			}) }),
			related.length > 0 && /* @__PURE__ */ jsx("section", {
				className: "border-t border-hairline",
				children: /* @__PURE__ */ jsxs(Container, {
					className: "py-16",
					children: [/* @__PURE__ */ jsx("p", {
						className: "label-tech mb-8",
						children: "Related work"
					}), /* @__PURE__ */ jsx("ul", { children: related.map((item, i) => /* @__PURE__ */ jsx("li", {
						className: "border-t border-hairline last:border-b",
						children: /* @__PURE__ */ jsxs(Link, {
							to: "/work/$slug",
							params: { slug: item.slug },
							className: "flex flex-wrap items-baseline justify-between gap-3 py-5",
							children: [/* @__PURE__ */ jsxs("span", {
								className: "font-display text-xl font-semibold",
								children: [
									String(i + 1).padStart(2, "0"),
									"  ",
									item.title
								]
							}), /* @__PURE__ */ jsx("span", {
								className: "text-sm text-quiet",
								children: item.category
							})]
						})
					}, item.slug)) })]
				})
			}),
			/* @__PURE__ */ jsx("section", {
				className: "border-t border-hairline",
				children: /* @__PURE__ */ jsxs(Container, {
					className: "flex flex-col gap-6 py-16 md:flex-row md:items-center md:justify-between",
					children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h2", {
						className: "font-display text-3xl font-semibold",
						children: "Start a similar project"
					}), /* @__PURE__ */ jsx("p", {
						className: "mt-2 text-sm text-quiet",
						children: "Tell us what you want to build."
					})] }), /* @__PURE__ */ jsx(ButtonLink, {
						to: "/contact",
						variant: "primary",
						children: "Start a Project"
					})]
				})
			}),
			next && /* @__PURE__ */ jsx("section", {
				className: "border-t border-hairline",
				children: /* @__PURE__ */ jsxs(Container, {
					className: "grid gap-8 py-12 md:grid-cols-12 md:items-center",
					children: [/* @__PURE__ */ jsxs("div", {
						className: "md:col-span-5",
						children: [/* @__PURE__ */ jsx("p", {
							className: "label-tech mb-3",
							children: "Next project"
						}), /* @__PURE__ */ jsxs(Link, {
							to: "/work/$slug",
							params: { slug: next.slug },
							className: "inline-flex items-center gap-3 font-display text-3xl font-semibold",
							children: [next.title, /* @__PURE__ */ jsx("span", {
								className: "btn-arrow",
								"aria-hidden": true,
								children: "→"
							})]
						})]
					}), /* @__PURE__ */ jsx("div", {
						className: "md:col-span-7",
						children: /* @__PURE__ */ jsx(Link, {
							to: "/work/$slug",
							params: { slug: next.slug },
							children: /* @__PURE__ */ jsx(ProjectVisual, {
								visual: next.visual,
								className: "aspect-[16/8]",
								interactive: false
							})
						})
					})]
				})
			})
		]
	});
}
//#endregion
export { CaseStudy as component };
